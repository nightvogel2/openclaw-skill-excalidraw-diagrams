    ...
    if label:
        ...
        if len(points) > 2:
            ...
            if abs(p2[0] - p1[0]) > abs(p2[1] - p1[1]):
                mid_y -= 25  # Horizontal segment - label above
            else:
                mid_x += 25  # Vertical segment - label to right
        else:
            # For straight arrows, offset perpendicular to arrow direction
            import math
            length = math.sqrt(dx*dx + dy*dy) or 1
            # Normal vector (perpendicular)
            nx = -dy / length
            ny = dx / length
            # Offset 25px perpendicular (always above/left)
            offset = 25
            mid_x = start_x + dx * 0.5 + nx * offset
            mid_y = start_y + dy * 0.5 + ny * offset
...
class Diagram:
    ...
    def box(
        ...
        # If fill_only mode, remove stroke (just show fill color)
        if getattr(self.style, 'fill_only', False):
            shape_elem["strokeWidth"] = 0
            shape_elem["strokeColor"] = "transparent"
    ...
    def arrow_between(
        ...
        if use_orthogonal:
            ...
            # Get the arrow element that was just added to set bindings
            arrow_elem = self.elements[-1] if self.elements else None
        else:
            ...
            # The arrow element is the first in the returned list
            arrow_elem = elems[0] if elems else None
        
        # Add arrow bindings to connect arrows properly to shapes
        if arrow_elem:
            # Determine shape types for binding
            source_shape_type = source.data.get("type", "rectangle")
            target_shape_type = target.data.get("type", "rectangle")
            
            # Set bindings to source and target shapes
            arrow_elem["startBinding"] = {
                "elementId": source.data["id"],
                "type": source_shape_type,
                "focus": 0,
                "gap": 5,
                "fixedPoint": None
            }
            ...
            # Register arrow in source shape's boundElements
            if source.data.get("boundElements") is None:
                source.data["boundElements"] = []
            source.data["boundElements"].append({"id": arrow_elem["id"], "type": "arrow"})
            
            # Register arrow in target shape's boundElements
            if target.data.get("boundElements") is None:
                target.data["boundElements"] = []
            target.data["boundElements"].append({"id": arrow_elem["id"], "type": "arrow"})
...
# ============================================================================
# Specialized Diagram Classes (Extended)
# ============================================================================

class SequenceDiagram(Diagram):
    """Specialized diagram for creating sequence diagrams (message flows between participants)."""

    def __init__(
        self,
        participant_spacing: int = 200,
        message_spacing: int = 60,
        **kwargs
    ):
        super().__init__(**kwargs)
        self.participant_spacing = participant_spacing
        self.message_spacing = message_spacing
        self._participants: dict[str, dict] = {}  # id -> {element, x, label}
        self._next_participant_x = 100
        self._message_y = 200
        self._lifelines: dict[str, Element] = {}  # participant_id -> lifeline element
    ...
    def participant(
        ...
        Args:
            ...
            label: Display name of the participant
            color: Color for the participant box
            
        Returns:
            Element representing the participant
        """
        elem = self.box(
            self._next_participant_x, 50, 
            label, 
            width=150, 
            height=60, 
            color=color,
        ...
        self._participants[participant_id] = {
            "element": elem,
            "x": self._next_participant_x + 75,  # center x
            "label": label
        }
        
        # Create lifeline (vertical dashed line)
        lifeline_start_y = elem.y + elem.height + 10
        lifeline_end_y = self._message_y + 500  # Extended lifeline
        
        lifeline = self._create_lifeline(
            self._next_participant_x + 75, lifeline_start_y,
            self._next_participant_x + 75, lifeline_end_y
        )
        self.elements.append(lifeline)
        self._lifelines[participant_id] = lifeline
        
        self._next_participant_x += self.participant_spacing
        return elem
    ...
    def message(
        ...
        Args:
            from_id: Source participant ID
            to_id: Target participant ID
            label: Message label
            style: Line style (solid or dashed)
        """
        if from_id not in self._participants or to_id not in self._participants:
            raise ValueError(f"Participant {from_id} or {to_id} not found")
            
        from_x = self._participants[from_id]["x"]
        to_x = self._participants[to_id]["x"]
        
        # Create arrow
        arrow_elem = self._create_arrow(
            from_x, self._message_y,
            to_x, self._message_y,
            style=style
        )
        ...
        self._message_y += self.message_spacing

    def self_message(self, participant_id: str, label: str) -> None:
        """Add a self-message (loop back to same participant).
        ...
        part_x = self._participants[participant_id]["x"]
        loop_width = 80
        
        # Create self-loop arrow (rectangle path)
        points = [
            [0, 0],
            [loop_width, 0],
            [loop_width, 30],
        ...
        elem["startArrowhead"] = None
        elem["endArrowhead"] = "arrow"
        self.elements.append(elem)
        
        # Add label
        text_elem = text(
            part_x + loop_width + 10, self._message_y + 15,
            label,
            font_size=14,
            font_family=self.box_style.font_family,
            color="black"
    ...
    def note(
        ...
        """Add a note beside a participant.
        
        Args:
            participant_id: Participant ID to attach note to
            note_text: Note text content
            position: Position relative to participant (left or right)
        """
        if participant_id not in self._participants:
            raise ValueError(f"Participant {participant_id} not found")
        ...
        if position == "right":
            ...
            note_x = part_x - note_width - 50
            
        note_y = self._message_y
        
        # Create note box with yellow background
        note_elem = self.box(
            note_x, note_y,
            note_text,
            width=note_width,
        ...
        # Connect note to participant with dashed line
        line_elem = self._create_arrow(
            part_x, note_y + note_height/2,
            note_x + (note_width/2 if position == "right" else note_width/2), note_y + note_height/2,
            style="dashed",
            arrowhead=None
        )
        self.elements.append(line_elem)
        
        self._message_y += max(note_height + 20, self.message_spacing)

    def _create_lifeline(self, x1: float, y1: float, x2: float, y2: float) -> dict:
        """Create a dashed vertical lifeline."""
        elem = _base_element(
            "line", x1, y1, 0, y2 - y1,
            stroke_color=COLORS["gray"],
            bg_color="transparent",
            roughness=self.style.roughness,
            stroke_style="dashed",
            stroke_width=1,
        )
        elem["points"] = [[0, 0], [x2 - x1, y2 - y1]]
        return elem
    ...
    def save(self, path: Union[str, Path]) -> Path:
        """Save the sequence diagram with trimmed lifelines."""
        # Trim lifelines to end 40px below the last message
        max_y = self._message_y + 40
        for pid, lifeline in self._lifelines.items():
            # Update the lifeline's end y to max_y
            start_y = self._participants[pid]["element"].y + self._participants[pid]["element"].height + 10
            lifeline["points"] = [[0, 0], [0, max_y - start_y]]
            lifeline["height"] = max_y - start_y
        return super().save(path)


class MindMap(Diagram):
    """Specialized diagram for creating mind maps (radial concept maps)."""

    def __init__(
        self,
        center_x: int = 500,
        center_y: int = 400,
        branch_length: int = 300,
        **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self._nodes: dict[str, dict] = {}  # id -> {element, parent_id, angle, level}
        self._central_node = None
        self._branch_angles = {}  # parent_id -> list of used angles
        
    def central(self, label: str, color: str = "blue") -> Element:
        """Create the central node of the mind map.
        
        Args:
            label: Central concept label
            color: Color of the central node
            
        Returns:
            Element representing the central node
        """
        elem = self.box(
        ...
        self._central_node = elem
        self._nodes["__central__"] = {
            "element": elem,
            "parent_id": None,
            "angle": 0,
            "level": 0
        }
        self._branch_angles["__central__"] = []
        
        return elem

    def branch(self, parent_id: str, label: str, color: str = "green") -> str:
        """Add a branch from a parent node.
        
        Args:
            parent_id: ID of the parent node
            label: Branch label
            color: Color of the branch node
            
        Returns:
            ID of the created branch node
        """
        if parent_id not in self._nodes:
            raise ValueError(f"Parent node {parent_id} not found")
            
        parent_info = self._nodes[parent_id]
        parent_elem = parent_info["element"]
        parent_level = parent_info["level"]
        
        # Calculate angle for this branch
        angle = self._get_next_angle(parent_id)
        distance = self.branch_length * (0.85 ** parent_level)
        
        # Calculate position
        branch_x = parent_elem.center_x + distance * math.cos(angle)
        branch_y = parent_elem.center_y + distance * math.sin(angle)
        
        # Auto-size box to fit label text
        char_width = 10  # approximate pixels per character
        text_width = len(label) * char_width + 30  # padding
        node_width = max(text_width, 120) if parent_level == 1 else max(text_width, 100)
        node_height = 50 if parent_level <= 1 else 40
        
        elem = self.box(
            branch_x - node_width/2, branch_y - node_height/2,
            label,
            width=node_width,
            height=node_height,
            color=color,
        ...
        self.arrow_between(
            parent_info["element"], elem,
            routing="straight",
            color="gray"
        )
        
        # Generate unique ID and store node info
        branch_id = f"branch_{len(self._nodes)}"
        self._nodes[branch_id] = {
            "element": elem,
            "parent_id": parent_id,
            "angle": angle,
            "level": parent_level + 1
        }
        self._branch_angles[branch_id] = []
        
        return branch_id

    def leaf(self, parent_id: str, label: str) -> str:
        """Add a leaf node (terminal node).
        ...
        Returns:
            ID of the created leaf node
        """
        return self.branch(parent_id, label, color="orange")

    def _get_next_angle(self, parent_id: str) -> float:
        """Calculate the next available angle for a child branch."""
        used_angles = self._branch_angles[parent_id]
        
        if parent_id == "__central__":
            # For central node, assign branches to quadrants in order to avoid overlapping
            n = len(used_angles)
            # Evenly space around the circle, starting from right (0 radians)
            # Use 8 slots for good distribution: right, down-right, down, down-left, left, up-left, up, up-right
            preferred_angles = [0, math.pi/4, math.pi/2, 3*math.pi/4, 
                              math.pi, 5*math.pi/4, 3*math.pi/2, 7*math.pi/4]
            # Use the next available angle from the sequence
            if n < len(preferred_angles):
                angle = preferred_angles[n]
            else:
                # Fallback: distribute evenly around the circle
                angle = (2 * math.pi * n) / 8
        else:
            parent_angle = self._nodes[parent_id]["angle"]
            n_siblings = len(used_angles)
            # Fan spread: +/- 36 degrees from parent angle, evenly distributed
            spread = math.pi / 5  # 36 degrees each side
            if n_siblings == 0:
                angle = parent_angle
            else:
                # Alternate left/right
                offset = ((n_siblings + 1) // 2) * spread / 2
                if n_siblings % 2 == 0:
...
class TimelineDiagram(Diagram):
    """Specialized diagram for creating timeline diagrams."""

    def __init__(
        self,
        orientation: Literal["horizontal", "vertical"] = "horizontal",
        timeline_length: int = 800,
        title: str = None,
        **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.title = title
        self._events: List[dict] = []  # {date_label, title, description, color, type}
        self._timeline_drawn = False
        
        # Timeline positioning
        if orientation == "horizontal":
            self.timeline_x = 100
            self.timeline_y = 300
            self.timeline_end_x = self.timeline_x + timeline_length
            self.timeline_end_y = self.timeline_y
        else:
            self.timeline_x = 200
            self.timeline_y = 100
            self.timeline_end_x = self.timeline_x
            self.timeline_end_y = self.timeline_y + timeline_length
    ...
    def period(
        ...
        """Add a period span to the timeline.
        
        Args:
            start_label: Start date label
            end_label: End date label
            title: Period title
            color: Period color
        """
        self._events.append({
    ...
    def build(self) -> None:
        """Build the timeline diagram (call this after adding all events)."""
        if not self._events:
            return
            
        # Add title if one was set
        if self.title:
            if self.orientation == "horizontal":
                title_x = self.timeline_x + self.timeline_length / 2
                title_y = self.timeline_y - 80
            else:
                title_x = self.timeline_x - 100
                title_y = self.timeline_y + self.timeline_length / 2
            
            title_elem = self.text_box(
                title_x - 100, title_y,  # Center the title
                self.title,
                font_size=24,
                color="black"
            )
            
        # Sort events by date_label for chronological order
        self._events.sort(key=lambda e: e["date_label"])
        
        # Draw timeline backbone
        self._draw_timeline_backbone()
        
        # Position and draw events
        event_spacing = self.timeline_length / max(len(self._events), 1)
        
        for i, event in enumerate(self._events):
            if self.orientation == "horizontal":
                event_x = self.timeline_x + i * event_spacing
                event_y = self.timeline_y
            else:
                event_x = self.timeline_x
                event_y = self.timeline_y + i * event_spacing
                
            self._draw_event(event, event_x, event_y, i % 2)

    def _draw_timeline_backbone(self) -> None:
        """Draw the main timeline line."""
        line_elem = _base_element(
            "line", 
        ...
        line_elem["points"] = [[0, 0], [self.timeline_end_x - self.timeline_x, self.timeline_end_y - self.timeline_y]]
        self.elements.append(line_elem)

    def _draw_event(self, event: dict, x: float, y: float, alternate: bool) -> None:
        """Draw a single event on the timeline."""
        event_type = event["type"]
        color = event["color"]
        
        if event_type == "milestone":
            # Diamond shape for milestones - larger size (50x50)
            elem = self.box(
                x - 25, y - 25,
                "",  # No label inside diamond
                width=50, height=50,
                color=color,
                shape="diamond"
            )
            # Use full stroke color for milestones, not just pastel fill
            if elem.data.get("strokeColor"):
                elem.data["strokeColor"] = COLORS.get(color, color)
        else:
            # Circle for events - larger size (35x35)
            elem = self.box(
                x - 17.5, y - 17.5,
                "",  # No label inside circle
                width=35, height=35,
                color=color,
                shape="ellipse"
            )
        
        # Add connector line from backbone to event marker
        if self.orientation == "horizontal":
            connector_start_y = self.timeline_y
            connector_end_y = y
            connector_line = line(x, connector_start_y, x, connector_end_y, color="gray")
        else:
            connector_start_x = self.timeline_x
            connector_end_x = x
            connector_line = line(connector_start_x, y, connector_end_x, y, color="gray")
        self.elements.append(connector_line)
        
        # Add event details with increased text offsets
        if self.orientation == "horizontal":
            text_y = y + (60 if alternate else -80)  # Increased from 40/-60
            text_x = x - 50
        else:
            text_x = x + (60 if alternate else -150)
            text_y = y - 20
            
        # Event title with larger font size (18px)
        title_elem = text(
            text_x, text_y,
            f"{event['date_label']}\n{event['title']}",
            font_size=18,  # Increased from 12
            font_family=self.box_style.font_family,
            color="black"
        )
        self.elements.append(title_elem)
        
        # Description if provided with larger font size (13px)
        if event["description"]:
            desc_elem = text(
                text_x, text_y + 30,
                event["description"],
                font_size=13,  # Increased from 10
                font_family=self.box_style.font_family,
                color="gray"
            )
            self.elements.append(desc_elem)

    def save(self, path: str) -> str:
        """Build the timeline and save. Auto-calls build() if not already built."""
        if not self._timeline_drawn:
            self.build()
            self._timeline_drawn = True
        return super().save(path)


class ERDiagram(Diagram):
    """Specialized diagram for creating Entity-Relationship diagrams."""

    def __init__(
        self,
        entity_spacing: int = 250,
        **kwargs
    ):
        super().__init__(**kwargs)
        self.entity_spacing = entity_spacing
        self._entities: dict[str, Element] = {}  # entity_id -> Element
        self._next_x = 100
        self._next_y = 100
    ...
    def entity(
        ...
        Returns:
            Element representing the entity
        """
        attributes = attributes or []
        
        # Create entity box content
        content = name
        if attributes:
            content += "\n" + "-" * len(name) + "\n" + "\n".join(attributes)
        ...
        height = max(80, line_count * 20 + 20)
        width = max(150, len(name) * 10, max(len(attr) * 8 for attr in attributes) if attributes else 0)
        ...
        self._entities[entity_id] = elem
        
        # Update position for next entity (grid layout)
        self._current_row_height = max(self._current_row_height, height + 50)
        self._next_x += width + self.entity_spacing
        
        # Wrap to next row if needed
        if self._next_x > 800:
            self._next_x = 100
            self._next_y += self._current_row_height
            self._current_row_height = 0
    ...
    def relationship(
        ...
        Args:
            ...
            cardinality: Relationship cardinality (e.g., "1:1", "1:N", "M:N")
        """
        if entity1_id not in self._entities or entity2_id not in self._entities:
            raise ValueError(f"Entity {entity1_id} or {entity2_id} not found")
            
        entity1 = self._entities[entity1_id]
        entity2 = self._entities[entity2_id]
        
        # Create relationship diamond
        mid_x = (entity1.center_x + entity2.center_x) / 2
        mid_y = (entity1.center_y + entity2.center_y) / 2
        ...
        self.arrow_between(entity1, rel_elem, routing="straight", color="black")
        self.arrow_between(rel_elem, entity2, routing="straight", color="black")
        
        # Add cardinality labels
        # Parse cardinality (e.g., "1:N" -> "1" and "N")
        if ":" in cardinality:
            card1, card2 = cardinality.split(":", 1)
            
            # Position cardinality labels near entity connections
            card1_x = entity1.center_x + (rel_elem.center_x - entity1.center_x) * 0.8
            card1_y = entity1.center_y + (rel_elem.center_y - entity1.center_y) * 0.8
            
            card2_x = entity2.center_x + (rel_elem.center_x - entity2.center_x) * 0.8
            card2_y = entity2.center_y + (rel_elem.center_y - entity2.center_y) * 0.8
            
            card1_elem = text(
                card1_x, card1_y - 10, card1,
                font_size=12, font_family=self.box_style.font_family,
                color="black"
            )
            card2_elem = text(
                card2_x, card2_y - 10, card2,
                font_size=12, font_family=self.box_style.font_family,
                color="black"
            )
            
            self.elements.append(card1_elem)
            self.elements.append(card2_elem)
